...
class SubmissionService(
    ...
    fun createWithImages(
        ...
        return transaction {
            ...
            try {
                ...
                val (submission, id) = submissionRepository.create(clientId)
                submissionId = id
                ...
                val (imageMessages, submissionResponse) = submissionRepository.getSubmissionWithImages(submissionId)
    ...
    fun getById(id: Int): Pair<HttpStatusCode, BaseResponse<SubmissionDetailResponse?>> {
        return transaction {
            val submission = submissionRepository.findById(id)?.toDetailResponse()
            if (submission != null) {
    ...
    fun review(
        ...
        return transaction {
            try {
                ...
                val updatedSubmission = submissionRepository.findById(id)?.toResponse()!!
                HttpStatusCode.OK to BaseResponse(
                    success = true,
                    code = HttpStatusCode.OK.value,
        ...
        return transaction {
            try {
                ...
                HttpStatusCode.OK to BaseResponse(
                    success = true,
                    code = HttpStatusCode.OK.value,
                    data = history.map { it.toResponse() }
                )
            } catch (e: Exception) {
                HttpStatusCode.InternalServerError to BaseResponse(
                    success = false,
