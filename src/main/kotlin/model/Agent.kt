 ...
 */
@OptIn(ExperimentalTime::class)
fun Agent.toAgentResponse(): AgentResponse {
    return transaction {
        AgentResponse(
            id = this@toAgentResponse.id.value,
            name = this@toAgentResponse.name,
            username = this@toAgentResponse.username,
            createdAt = this@toAgentResponse.createdAt,
            updatedAt = this@toAgentResponse.updatedAt
        )
    }
}
 ...
 */
@OptIn(ExperimentalTime::class)
fun List<Agent>.toAgentResponses(): List<AgentResponse> {
    return transaction {
        this@toAgentResponses.map { agent ->
            AgentResponse(
                id = agent.id.value,
                name = agent.name,
